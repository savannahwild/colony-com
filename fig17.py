# -*- coding: utf-8 -*-
"""
Created on Tue May  4 09:07:52 2021
...
## experimental parameters
    ...
    concsS=[0.01,0.1,1,1,1]
    concsR=[1,1,1,0.1,0.01]

    for col in range(len(concsS)):
            ...
            for i in np.linspace(29, 29, 1):
                U_S[int(i), int(i)] = 0.001*concsS[col]
            S = Species("S", U_S)
            def S_behaviour(species, params):
                ## unpack params
                D, rho_n, Dc, rc, w, rho_A, Da, Dt = params
                s = Dc * hf.ficks(species['S'], w)*hf.leaky_hill(s=species['T']+species['A'], K=40e-9, lam=2, max=2.75, min=1) + species['S']*hf.leaky_hill(s=species['N'], K=80, lam = 1, max=rc, min=0)
                return s
            ...
            for i in np.linspace(29, 29, 1):
                for j in np.linspace(29,29, 1):
                    U_R[int(i), int(j)] = 0.001*concsR[col]
            R = Species("R", U_R)
            ...
            plate.plot_conc_target(sim, S, 10, col)
