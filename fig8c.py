# -*- coding: utf-8 -*-
"""
Created on Mon May  3 00:49:21 2021

@author: savan
"""

#Basic model, sender minority
...
## add sender strain to the plate
    ...
    for col, conc in enumerate(concs):
        ...
        def N_behaviour(species, params):
            ...
            n = D*hf.ficks(species['N'], w) - (species['R']+species['S'])*rho_n*hf.leaky_hill(s=species['N'], K=0.02, lam=1, max=rc, min=0) #rho_n * species['N'] * (species['R'])
            return n
        ...
        def S_behaviour(species, params):
            ## unpack params
            D, rho_n, Dc, rc, w, rho_A, Da, Dt = params
            s = Dc*hf.ficks(species['S'], w) + species['S']*hf.leaky_hill(s=species['N'], K=0.02, lam = 1, max=rc, min=0)
            return s
        ...
        for i in np.linspace(29, 29, 1):
            for j in np.linspace(29,29, 1):
                U_R[int(i), int(j)] = conc
        R = Species("R", U_R)
        def R_behaviour(species, params):
            ## unpack params
            D, rho_n, Dc, rc, w, rho_A, Da, Dt = params
            r = Dc*hf.ficks(species['R'], w) + species['R']*hf.leaky_hill(s=species['N'], K=0.02, lam = 1, max=rc, min=0)
            return r
